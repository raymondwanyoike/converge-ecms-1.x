...
import dk.i2m.converge.newsml.v1_0.*;
import java.io.File;
...
public class NewsMLDecoder implements NewswireDecoder {
    ...
    public enum Property {
        ...
        /** Mapping of attachment types to renditions in the attachment catalogue. */
     ...
     * @return {@code true} if the {@link Property} is set, otherwise {@code false}
     */
     ...
     * @return Value of the {@link Property} or {@code null} if the {@link Property} was not set
     */
    ...
    private void readLocations() {
        ...
        if (isPropertySet(Property.PROPERTY_NEWSWIRE_PROCESSED_LOCATION)) {
            moveProcessed = true;
            deleteProcessed = false;
            processedLocation = getProperty(
                    Property.PROPERTY_NEWSWIRE_PROCESSED_LOCATION);
        }
     ...
     * <p/>
     * @return {@code true} of the properties are sufficient to execute the decoder, otherwise {@code false}
     */
        ...
        if (moveProcessed) {
            if (!isPropertySet(Property.PROPERTY_NEWSWIRE_PROCESSED_LOCATION)) {
                ...
                if (!processNewswireDirectory.exists()) {
                    log(LogSeverity.SEVERE, "LOG_PROCESS_LOCATION_NOT_EXIST",
                            processedLocation);
                } else if (!processNewswireDirectory.canWrite()) {
                    log(LogSeverity.SEVERE, "LOG_PROCESS_LOCATION_NOT_WRITABLE",
                            processedLocation);
                }
            }
        }
    ...
    private void readNewswires() {
        try {
            JAXBContext jc = JAXBContext.newInstance(JAXB_CONTEXT, getClass().
                    getClassLoader());
            ...
            for (File file : xmlFilesToProcess) {
                ...
                List<NewswireItem> results = pluginCtx.
                        findNewswireItemsByExternalId(file.getName());
                ...
                if (results.isEmpty()) {
                    ...
                    try {
                        Date itemDate = NEWSML_DATE_FORMAT.parse(newsMl.
                                getNewsEnvelope().getDateAndTime().getValue());
...
//                                            newswireItem.setSummary(summary.toString().trim());
                                        ...
                                        for (ContentItem picContentItem :
                                            ...
                                            if (picContentItem.getHref() != null) {
                                                for (dk.i2m.converge.newsml.v1_0.Property p :
                                                        picContentItem.
                                                        getCharacteristics().
                                                        getProperty()) {
                                                    if (p.getFormalName().
                    ...
                    if (fileMissing) {
                        ...
                        NewswireItem nwi = pluginCtx.createNewswireItem(
                                newswireItem);
