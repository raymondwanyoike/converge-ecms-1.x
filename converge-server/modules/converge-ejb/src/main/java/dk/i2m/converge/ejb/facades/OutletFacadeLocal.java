...
public interface OutletFacadeLocal {
     ...
     *
     * @param edition
     *          {@link Edition} to create
     */
    Edition createEdition(Edition edition);

    Edition createEdition(Long outletId, Boolean open, Date publicationDate, Date expirationDate, Date closeDate);
    ...
    /**
     * Deletes an existing {@link Department} by its unique <code>id</code>.
     *
     * @param id
    ...
    /**
     * Delete an existing {@link Edition} by its unique <code>id</code>.
     *
     * @param id
    ...
    /**
     * Finds a {@link Department} by its unique <code>id</code>.
     *
     * @param if
     * Unique id of the {@link department}
     * @return {@link Department} matching the <code>id</code>
    ...
    /**
     * Finds an {@link Edition} by its unique <code>id</code>.
     *
     * @param id
     * Unique id of the {@link Edition}
     * @return {@link Edition} matching the <code>id</code>
     * @throws DataNotFoundException
     ...
     *         filtered by <code>status</code>
     */
    List<Edition> findEditionsByStatus(boolean status, Outlet outlet);

    /**
     * Finds the next edition of a given {@link Outlet}. The next possible edition
     * is only sought 1 month ahead. That means, if the date today is 1st January,
     ...
     * @return {@link List} of {@link EditionCandidate}s for the given {@link Outlet} and
     *         date
     */
    List<EditionCandidate> findEditionCandidatesByDate(Outlet outlet, Calendar date, boolean includeClosed);
     ...
     * @return {@link Outlet} matching the <code>id</code>
    ...
    Edition updateEdition(Long editionId, Boolean open, Date publicationDate, Date expirationDate, Date closeDate) throws DataNotFoundException;
